...
GRACE_DAYS = 28          # nothing is judged before this — indexing is slow, and
                         # this site is new enough that four weeks is the first
                         # point at which "no traffic" means anything at all
WINDOW = 14              # trailing window for "is it working now"
MIN_TOTAL_VISITORS = 10  # cumulative owned visitors needed to call it alive
...
# rather than merely un-clicked. Deliberately low: the whole site earns a
# handful of impressions a week, so a technique whose pages Google serves at
# all is carrying a real signal. Above this bar a technique that misses the
# visitor threshold is flagged for a rewrite, not retired — retiring a page
# that ranks for a query nobody types is right, retiring one Google serves but
# we title badly throws away the work at the moment it started paying.
MIN_OWNED_IMPRESSIONS = 3

# The consequence of the guard below, stated plainly because it is easy to read
# this module and expect pruning that will not happen: while Search Console is
# unconnected, `recent_visibility` reports `measured: False` and NOTHING is
# ever retired on traffic. That is the intended behaviour — a technique judged
# dead by a metric with no resolution is a technique deleted for no reason —
# and it means connecting Search Console is what turns this from an automation
# into a loop that improves.
