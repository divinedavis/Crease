...
# Numbers the model must not have changed. Checked as substrings of the whole
# rendered text: if a price appears at all, it must be one of ours.
PRICE_RE = re.compile(r"\$\s?\d[\d,]*(?:\.\d{2})?")
ALLOWED_PRICES = {facts.PRICE_PER_POUND, facts.MINIMUM,
                  facts.COURIER_ROUND_TRIP, facts.COURIER_ONE_LEG}
...
def validate(doc, query):
    ...
    for found in PRICE_RE.findall(text):
        ...
        if normalised not in ALLOWED_PRICES:
            raise Rejected(f"draft states a price we do not charge: {found!r}")
